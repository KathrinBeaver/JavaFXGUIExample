package sample.components.calendarview;
...
class HeadSegment implements Segment {
    ...
    public HeadSegment() { this.position = new Position(4, 4); }
    ...
    public void draw(CalendarEntry[][] calendarEntries) {
        CalendarEntry correspondingElement =
            calendarEntries[this.position.getY()][this.position.getX()];
    ...
    public boolean isOutOfBounds(Position rightDownCorner) {
        return this.position.getX() < 0 ||
            this.position.getX() > rightDownCorner.getX() ||
            this.position.getY() < 0 ||
            this.position.getY() > rightDownCorner.getY();
    }
}
...
class BodySegment implements Segment {
    ...
    BodySegment(Position position) { this.position = position; }
...
class FoodItem {
    ...
    public void draw() { this.entry.setBackgroundColor("rgb(128, 128, 0)"); }

    public void hide() { this.entry.hideText(); }
...
class FoodSystem {
    ...
    public void selectRandomFood() {
        int index =
            (int)Math.floor(Math.random() * this.availableSources.size());
    ...
    public void draw() { this.currentlyActive.draw(); }
}
...
class Snake {
    ...
    public void draw(CalendarEntry[][] calendarEntries) {
        Arrays.stream(calendarEntries)
            .forEach(row
                     -> Arrays.stream(row).forEach(
                         item -> item.clearBackgroundStyle()));
        this.segments.forEach(segment -> segment.draw(calendarEntries));
    }

    public void update() {
        for (int i = 1; i < this.segments.size(); ++i) {
            this.segments.get(this.segments.size() - i)
                .setPosition(this.segments.get(this.segments.size() - i - 1)
                                 .getPosition());
        }
...
public class Game {
    ...
    public Game(CalendarEntry[][] calendarEntries, Label extraLabel) {
        ...
        this.bottomRightCorner = new Position(calendarEntries[0].length - 1,
                                              calendarEntries.length - 1);
        this.extraLabel = extraLabel;
    }

    public void start() {
        Arrays.stream(this.calendarEntries)
            .forEach(
                row
                -> Arrays.stream(row).forEach(element -> element.unhideText()));
        Timeline timeline = new Timeline(
            new KeyFrame(Duration.seconds(1), new EventHandler<ActionEvent>() {
                ...
                public void handle(ActionEvent actionEvent) {
                    ...
                    if (snake.isCollidingFood(
                            foodSystem.getCurrentActiveFoodPosition())) {
                        foodSystem.eat();
                    }
