...
public class SnakeCalendar extends GridPane {
    ...
    public SnakeCalendar() {
        ...
        this.calendarEntries =
            new CalendarEntry[CALENDAR_ENTRY_HEIGHT][CALENDAR_ENTRY_WIDTH];
    ...
    private GridPane getGridPane(int borderWidth) {
        ...
        for (int i = 0; i < this.CALENDAR_ENTRY_WIDTH; ++i) {
            for (int j = 0; j < this.CALENDAR_ENTRY_HEIGHT; ++j) {
                CalendarEntry entry =
                    new CalendarEntry(data.getDateString(i, j));
...
class CalendarEntry extends Label {
    ...
    private String getRandomColor() {
        return String.format("rgb(%d, %d, %d)", this.getRandomColorValue(),
                             this.getRandomColorValue(),
                             this.getRandomColorValue());
    }
    ...
    public void unhideText() { this.setText(this.text); }
}
...
class CalendarData {
    ...
    public String getDateString(int i, int j) {
        return this.dateNumbers[j][i] == 0
            ? ""
            : Integer.toString(this.dateNumbers[j][i]);
    }
